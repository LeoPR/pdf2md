...
**replicado em 3 VLMs** (Nougat/GOT/PaddleOCR-VL). O auditor credita a fidelidade real
dos VLMs no terreno deles (não é enviesado), distingue pelos 2 eixos uma falha
"honesto-esparsa" de uma "alucinação-rica", e **nunca foi enganado p/ cima** (15 casos).
**Limite medido (onda 2c):** em scan archaico onde o próprio leitor OCR da referência
falha, a régua pode FALSO-penalizar uma extração boa — o auditor vale em renders
OCR-legíveis (born-digital). Falta promover `fidelity_report()` com um gate de confiança
da referência.
