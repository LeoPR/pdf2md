"""Relatório de telemetria de uma extração: tokens, math, imagens, round-trip.

Saídas:
- `_stats.json` (machine-readable, completo)
- `_stats.md`   (human-readable, relatório)

Pode ser usado como módulo:
    from pdf2md.stats import compute_stats
    compute_stats(folder, source_pdf=pdf, tool_overrides={"marker_version": "1.10.2"})

Ou como script standalone (compat):
    python src/stats.py FOLDER [--source-pdf PDF] [--roundtrip-md1 A --roundtrip-md2 B]
"""
...
def detect_tools(
    ...
    Precedência: kwargs explícitos > env vars `PDF2MD_<TOOL>_VERSION` > auto-detect.
    O caso típico vindo do `pdf2md.cli` é via kwargs (cli já detectou versões reais
    do venv do marker via inspect); env vars existem para back-compat com chamada
    standalone antiga (`python src/stats.py`).
    """
    info = {
    ...
    if marker_version:
        info["marker"] = marker_version
    elif os.environ.get("PDF2MD_MARKER_VERSION"):
    ...
    if torch_version:
        ...
        if cuda_device:
            ...
            info["cuda_device"] = cuda_device
            if cuda_memory_gb is not None:
                info["cuda_memory_gb"] = float(cuda_memory_gb)
        else:
            info["cuda_available"] = False
    elif os.environ.get("PDF2MD_TORCH_VERSION"):
    ...
    elif os.environ.get("PDF2MD_PANDOC_VERSION"):
...
def md_metrics(md_path: Path) -> dict:
    ...
    # LaTeX command frequency (apenas dentro de math markers, p/ não contar markdown como \n)
...
def folder_metrics(folder: Path) -> dict:
    ...
    if chs:
        ...
        out["totals"] = {
            ...
            "images_by_format": dict(agg_format),
            "images_bytes_by_format": agg_format_bytes,
...
def roundtrip_metrics(md1: Path, md2: Path) -> dict:
    ...
    # T071: bloat_ratio detecta padrão de alucinação no re-OCR do PDF intermediário.
...
def render_md(stats: dict) -> str:
    ...
    lines += [
        ...
        "  ```",
        "",
        "Após extrair, rodar `pdf2md restruct` (livro) ou mover MD direto (paper).",
        "",
    ]
...
def compute_stats(
    ...
    roundtrip_md1: Path | None = None,
    roundtrip_md2: Path | None = None,
    extraction_time_seconds: float | None = None,
    tools: dict | None = None,
    tool_overrides: dict | None = None,
) -> tuple[Path, Path]:
    """Pipeline completo: coleta métricas, salva `_stats.json` + `_stats.md` na pasta.

    Args:
        md_folder: pasta com MDs (book layout ou paper flat).
        source_pdf: PDF original (opcional, p/ pdf_metrics).
        roundtrip_md1, roundtrip_md2: par MD inicial / pós-roundtrip (opcional).
        extraction_time_seconds: tempo de extração em s (opcional).
        tools: dict já pronto de versões (se fornecido, pula detect_tools).
        tool_overrides: kwargs p/ `detect_tools` (precedência sobre env vars).

    Retorna `(json_path, md_path)`.
    """
    if tools is None:
        tools = detect_tools(**(tool_overrides or {}))

    stats = {
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "tools": tools,
        "source": pdf_metrics(source_pdf) if source_pdf else {},
        "output": folder_metrics(md_folder),
    }
    if extraction_time_seconds:
        stats["extraction_time_seconds"] = extraction_time_seconds
    if roundtrip_md1 and roundtrip_md2:
        stats["roundtrip"] = roundtrip_metrics(roundtrip_md1, roundtrip_md2)

    json_path = md_folder / "_stats.json"
    md_path = md_folder / "_stats.md"
...
def _cli() -> int:
    """CLI standalone (compat com `python src/stats.py FOLDER [...]`)."""
    import argparse
    ...
    json_path, md_path = compute_stats(
        ...
        roundtrip_md1=args.roundtrip_md1,
        roundtrip_md2=args.roundtrip_md2,
        extraction_time_seconds=args.extraction_time,
    )
